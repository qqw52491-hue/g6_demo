      ...
      <aside class="sidebar glass-panel">
        ...
        <div class="control-section">
          ...
          <div class="slider-group">
            ...
            <input 
              type="range" v-model.number="businessData.node1.cpu" 
              min="0" max="100"
            >
          </div>
           <div class="slider-group">
            ...
            <input 
              type="range" v-model.number="businessData.node2.cpu" 
              min="0" max="100"
            >
          </div>
        </div>
        ...
        <div class="control-section">
          ...
          <div class="button-group">
            <button 
              :class="{ active: businessData.selectedId === 'node1' }"
              @click="selectNode('node1')"
            >
              Select Node 1
            </button>
            <button 
              :class="{ active: businessData.selectedId === 'node2' }"
              @click="selectNode('node2')"
            >
              Select Node 2
            </button>
          </div>
        ...
        <!-- Scenario 3: Global Mode -->
         <div class="control-section">
          ...
          <button 
              :class="{ active: businessData.isMaintenance }"
...
.main-content { display: flex; flex: 1; gap: 24px; overflow: hidden; }
...
/* Sidebar */
.sidebar { width: 340px; padding: 24px; display: flex; flex-direction: column; gap: 24px; overflow-y: auto; }
.sidebar h2 { font-size: 1.1rem; margin-top: 0; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 12px; }

.control-section { background: rgba(0,0,0,0.2); padding: 16px; border-radius: 8px; }
.control-section h3 { font-size: 0.85rem; color: #aaa; margin: 0 0 8px 0; text-transform: uppercase; letter-spacing: 0.5px; }
.desc { font-size: 0.75rem; color: #666; margin-bottom: 12px; line-height: 1.4; }

.slider-group { display: flex; flex-direction: column; margin-bottom: 8px; }
.slider-group label { font-size: 0.8rem; color: #ccc; margin-bottom: 4px; }
...
button {
  ...
  color: #ddd; border-radius: 6px; cursor: pointer; transition: all 0.2s; font-size: 0.8rem;
}
button:hover { background: rgba(255,255,255,0.1); }
button.active { background: #3B82F6; border-color: #3B82F6; color: white; }

/* Monitor */
.state-monitor { margin-top: auto; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.1); }
.monitor-item { font-size: 0.8rem; color: #888; margin-bottom: 6px; display: flex; justify-content: space-between; flex-wrap: wrap; }
.state-list { color: #fff; font-family: monospace; }

/* Canvas */
.canvas-wrapper { flex: 1; position: relative; background: radial-gradient(circle at center, #111, #000); }
.g6-mount { width: 100%; height: 100%; }
.overlay-hint { position: absolute; bottom: 16px; right: 16px; color: #555; font-size: 0.8rem; pointer-events: none; text-align: right; line-height: 1.5; }
</style>
